...
var conversionsReport = function () {
    _this.bodyData = '';
    _this.data = '';
    var conversionsReportModel = mongoose.model('conversionsReport', ConversionsReportSchema);
    request.get(conversionsReportUrl, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            _this.bodyData = JSON.parse(body);
            console.log(body);
            _this.getResolved();
        }
        else {
    ...
    _this.getResolved = function () {
        _this.data = new conversionsReportModel({
            response: JSON.stringify(_this.bodyData.response.data)
        });
        _this.data.save(function (err) {
            if (err)
                console.log(err);
            else
                console.log(session);
...
// conversionsReport();
TEMPReadStatsFromFile =  ()=> {
    var fs = require('fs');
    var obj = JSON.parse(fs.readFileSync('data.json', 'utf8')); //TODO get data from HO
    var arrOfObjsWithStat = obj.response.data.data;
    return _.map(arrOfObjsWithStat, (objWithStat)=> { return objWithStat.Stat; });
};
saveStats = (statsArr)=> {
    Stat.insertMany(statsArr, (err, docs)=> {
        if (err)
            console.log(err);
        else
            console.log("Succeeded saving " + docs.length + " docs");
    });
};
var stats = TEMPReadStatsFromFile();
// saveStats(stats);
        ...
        [
            {
                $group : {
                    _id : "$session_ip",
                    count: { $sum: 1 },
                    temp:{$push:{"ad_id":"$ad_id","country_code":"$country_code"}}
                }
            },
            {
                $match:{ count:{$gt:3}}
        ...
        [
            {
                $group : {
                    _id : "$ad_id",
                    count: { $sum: 1 },
                    temp:{$push:{"session_ip":"$session_ip","country_code":"$country_code"}}
                }
            },
            {
                $match:{ count:{$gt:1}}
